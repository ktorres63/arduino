...
#define OFFSETS  -698,    -408,    1324,      43,      50,       6 // mis ultimos offsets
...
int PrintValues(int32_t *quat, uint16_t SpamDelay = 100) { //no mover este
  Quaternion q;
  VectorFloat gravity;
  float ypr[3] = { 0, 0, 0 };
  float xyz[3] = { 0, 0, 0 };
...
void print_Values (int16_t *gyro, int16_t *accel, int32_t *quat, uint32_t *timestamp) { //mosstrar valores
  ...
  spamtimer(Spam_Delay){
    ...
    if (xyz[1] >10.0){
      Serial.println("Nariz arriba");  
    }
    else if(xyz[1] <-10.0){
      Serial.println("nariz abajo");
    }
  }
